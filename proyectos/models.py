...
class Actividad(models.Model):
    ...
    @property
    def porcentaje_avance_css(self):
        # devuelve string con punto y máximo 2 decimales
        if self.porcentaje_avance is None:
            return "0"
        return f"{float(self.porcentaje_avance):.2f}"
    # Control de Avance
    ...
    cantidad_ejecutada_total = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=0,
        verbose_name="Cantidad Ejecutada Total",
        help_text="Se actualiza automáticamente a partir de las actualizaciones de avance"
    )
    ...
    def esta_completada(self):
        ...
        return (self.porcentaje_avance or 0) >= 100

    def recalcular_cantidad_y_porcentaje(self):
        """
        Recalcula cantidad ejecutada total y porcentaje de avance desde los avances registrados.
        """
        total = self.avances.aggregate(
            total=models.Sum('cantidad_ejecutada')
        )['total'] or 0

        self.cantidad_ejecutada_total = total
        self.actualizar_porcentaje_avance()

    def actualizar_porcentaje_avance(self):
        """Actualiza porcentaje_avance según cantidad_programada y cantidad_ejecutada_total."""
        if self.cantidad_programada and self.cantidad_programada > 0:
            porcentaje = (self.cantidad_ejecutada_total / self.cantidad_programada) * 100
            ...
            self.porcentaje_avance = round(porcentaje, 2)
        else:
            # Si no hay cantidad programada, mantenemos el valor actual o lo dejamos en 0
            self.porcentaje_avance = self.porcentaje_avance or 0

        self.save(update_fields=['cantidad_ejecutada_total', 'porcentaje_avance'])

    def get_duracion_estimada_dias(self):
        """Duración estimada en días, según fechas programadas."""
        if self.fecha_inicio_estimada and self.fecha_fin_estimada:
            return (self.fecha_fin_estimada - self.fecha_inicio_estimada).days
        return None

    def get_duracion_real_dias(self):
        """Duración real en días, según fechas reales."""
        if self.fecha_inicio_real and self.fecha_fin_real:
            return (self.fecha_fin_real - self.fecha_inicio_real).days
        return None

    def get_dias_restantes_programado(self):
        """
        Días restantes según fecha fin estimada (programada).
        Mientras la actividad no tenga fecha fin real.
        """
        if self.fecha_fin_real:
            return None  # ya terminada
        if self.fecha_fin_estimada:
            return (self.fecha_fin_estimada - date.today()).days
        return None

    def esta_atrasada_programado(self):
        """
        True si, según la programación, la actividad está atrasada:
        - sin fecha fin real
        - hoy > fecha fin estimada
        """
        if not self.fecha_fin_real and self.fecha_fin_estimada:
            return date.today() > self.fecha_fin_estimada
        return False
    ...
    def retraso_dias(self):
        """
        Cuántos días de retraso tiene la actividad, basado en:
        - si tiene fecha fin real: fin_real - fin_estimada (si > 0)
        - si no tiene fin real y hoy > fin_estimada: hoy - fin_estimada
        """
        if self.fecha_fin_estimada:
            if self.fecha_fin_real:
                delta = (self.fecha_fin_real - self.fecha_fin_estimada).days
                return delta if delta > 0 else 0
            else:
                if date.today() > self.fecha_fin_estimada:
                    return (date.today() - self.fecha_fin_estimada).days
        return 0

    def necesita_atencion(self, umbral_dias=3):
        """
        True si:
        - faltan pocos días para la fecha fin estimada (<= umbral)
        - y el avance aún es menor al 100%
        """
        dias = self.get_dias_restantes_programado()
        if dias is None:
            return False
...
class AvanceActividad(models.Model):
    """
    Registro de avances parciales de una actividad.
    Permite controlar cantidades ejecutadas en el tiempo.
    """
    ...
    fecha_avance = models.DateField(
        verbose_name="Fecha del Avance"
    )

    cantidad_ejecutada = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        verbose_name="Cantidad Ejecutada en el Período"
    )
    ...
    def __str__(self):
        return f"Avance {self.cantidad_ejecutada} {self.actividad.unidad_medida} - {self.actividad}"

    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        # Recalcular totales y porcentaje de la actividad
        self.actividad.recalcular_cantidad_y_porcentaje()

    def delete(self, *args, **kwargs):
        actividad = self.actividad
        super().delete(*args, **kwargs)
        # Recalcular luego de eliminar
        actividad.recalcular_cantidad_y_porcentaje()
